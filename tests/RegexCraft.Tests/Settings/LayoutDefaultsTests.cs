...
public sealed class LayoutDefaultsTests
{
    ...
    {
        // Typical 1320 window body ≈ full width of MainBodyGrid
        const double body = 1320;
        var w = LayoutDefaults.ResolveCompareWidth(body, storedCompare: null);
        var content = body - LayoutDefaults.LeftSidebarWidth - 10;
        Assert.That(w, Is.EqualTo(content * LayoutDefaults.CompareShareOfBody).Within(1));
        Assert.That(w, Is.GreaterThan(content * 0.6), "Compare must take most of the content row");
        Assert.That(w, Is.GreaterThanOrEqualTo(LayoutDefaults.RightPanelCompareMin));
    }
    ...
    {
        // Old 520px default must not stick — expand to the share formula.
    ...
    {
        var w = LayoutDefaults.ResolveCompareWidth(1400, storedCompare: 900);
        Assert.That(w, Is.InRange(800, 1100));
    }
    ...
    public void ResolveRightPanelWidth_Compare_UsesBodyWhenProvided()
    {
        ...
        Assert.That(w, Is.GreaterThan(700));
    }
    ...
    public void ClampCompare_UsesCompareMinimum()
    {
        ...
        Assert.That(LayoutDefaults.ClampCompare(800), Is.EqualTo(800));
